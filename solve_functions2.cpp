...
{
	int q, y, z_m, m = arg->m, n = arg->n, k = arg->q, p = arg->p, r, z, j, q1 = arg->k;
	double c00 = 0.0, c01 = 0.0, c02 = 0.0, c10 = 0.0, c11 = 0.0, c12 = 0.0, c20 = 0.0, c21 = 0.0, c22 = 0.0;
	double *v_1 = arg->v1, *v_2 = arg->v2, *a = arg->a;
	int integer = s/p;
	...
	for( j = begin; j < k; j += p )
	{
		...
		for(r = 0; r < m_new; r += 3)
		{
			...
			for(y = 0; y < m_new; y += 3)
			{
				for(z = 0; z < m; z ++)
				{
					...
					c00 += v_1[r * m + z] * a[z_m + y];
					c01 += v_1[r * m + z] * a[z_m + y + 1];
					c02 += v_1[r * m + z] * a[z_m + y + 2];
	
					c10 += v_1[r * m + m + z] * a[z_m + y];
					c11 += v_1[r * m + m + z] * a[z_m + y + 1];
					c12 += v_1[r * m + m + z] * a[z_m + y + 2];
	
					c20 += v_1[r * m + 2*m + z] * a[z_m + y];
					c21 += v_1[r * m + 2*m + z] * a[z_m + y + 1];
					c22 += v_1[r * m + 2*m + z] * a[z_m + y + 2];
				}
		...
		for(y = 0; y < m_new; y += 3)
		{
			for(z = 0; z < m; z ++)
			{
				...
				c00 += v_1[m_new * m + z] * a[z_m + y];
				c01 += v_1[m_new * m + z] * a[z_m + y + 1];
				c02 += v_1[m_new * m + z] * a[z_m + y + 2];
		
				c10 += v_1[m_new * m + m + z] * a[z_m + y];
				c11 += v_1[m_new * m + m + z] * a[z_m + y + 1];
				c12 += v_1[m_new * m + m + z] * a[z_m + y + 2];
			...
			v_2[m_new * m + m + y]     = c10;
			v_2[m_new * m + m + y + 1] = c11;
			v_2[m_new * m + m + y + 2] = c12;
